...
## 前提・制約事項
- 従量課金機能につきましては、原則としてD推にて費用負担します。特定のリポジトリで高額な課金があった場合は別途お問い合わせさせていただくことがあります。
- 社外端末からの利用はできません。
...
- データ保存場所：米国
- アクセス認証：パスワード認証
- アクセス制限：クラウド型ID統合管理サービス(OneGate)
...
| CI/CD            | Actions       | 継続的インテグレーション・デリバリー（CI/CD）                        | 可  | 従量課金制           |
|                  | Packages      | パッケージのホスト・管理                                             | 可   | 従量課金制           |
| セキュリティ      | Advanced Security   | 静的コード解析/依存関係の脆弱性チェック/シークレットスキャン      | 可   | 従量課金制           |
